...
function setChooser() {
	...
	var value_ctl = document.getElementById('selNextOperator_TextBox')
	var chooser = new Actb(window._arrusers, value_ctl)
...
function Actb(ca) {
	var value_ctl = document.getElementById('selNextOperator_TextBox')
	this.actb_curr = document.getElementById('selNextOperator_TextBoxValue')
	...
	// this.actb_curr = jQuery('#'+value_ctl.id+'Value')[0];
	this.actb_curr.value_ctl = value_ctl

	if (this.actb_curr) {
		this.actb_curr.addEventListener(
			'focus',
			function (evte) {
				this.actb_curr = jQuery('#' + value_ctl.id + 'Value')[0]
				this.actb_curr.value_ctl = value_ctl
		...
		// jQuery(this.actb_curr).focus(function(){
		//     this.actb_curr = jQuery('#'+value_ctl.id+'Value')[0];
		//     this.actb_curr.value_ctl = value_ctl;
	...
	function actb_setup(curr_ctl) {
		...
		// jQuery(this.actb_curr).keydown(keydown_handler);
		// jQuery(this.actb_curr).blur(blur_handler);
		//jQuery(document).keypress(keypress_handler);
		// if (this.actb_curr.createTextRange) {debugger
	...
	function actb_clear(evt) {
		try {
			if (!evt) evt = event
			jQuery(this.actb_curr).unbind('keydown', keydown_handler)
			jQuery(this.actb_curr).unbind('blur', blur_handler)
			jQuery(document).unbind('keypress', keypress_handler)
			setTimeout(function () {
				actb_removedisp()
			}, 100)
		} catch (e) {}
	...
	function actb_generate() {
		...
		if (a) {
			...
			a.style.zIndex = 19891024
			a.style.top = eval(curTop(this.actb_curr) + this.actb_curr.offsetHeight) + 4 + 'px'
			a.style.left = this.actb_curr.getBoundingClientRect().left + $(document.body).scrollLeft() + 'px'
	...
	function actb_tocomplete(kc) {
		...
		if (actb_self.actb_delimiter.length > 0) {
			...
			for (i = 0, j = this.actb_curr.value.length; i < this.actb_curr.value.length; i++, j--) {
				// console.log( this.actb_curr.value.substr(i,j) );
				// console.log( this.actb_curr.value.substr(i,j).search(delim_split_rx) );
				if (this.actb_curr.value.substr(i, j).search(delim_split_rx) == 0) {
					//console.log( this.actb_curr.value.substr(i,j).match(delim_split_rx) );
					/*
        	  ma: [';', ';', ....]
        	*/
...
// For IE only, stops the obj = require(being selected
...
// Stop an event = require(bubbling up the event DOM
...
/* Offset position = require(top of the screen */
function curTop(obj) {
	toreturn = 0
	while (obj) {
...
// sets the caret selection = require(s to e in the object
